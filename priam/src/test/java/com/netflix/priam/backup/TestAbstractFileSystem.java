 ...
 * The actual upload/download of a file to remote file system is beyond the scope of this class.
 * Created by aagrawal on 9/22/18.
 */
public class TestAbstractFileSystem {
    private static final Logger logger = LoggerFactory.getLogger(TestAbstractFileSystem.class.getName());
    ...
    public void setBackupMetrics() {
        ...
        if (failureFileSystem == null)
            failureFileSystem = new FailureFileSystem(configuration, backupMetrics, backupNotificationMgr);
    ...
    private Collection<File> generateFiles(int noOfKeyspaces, int noOfCf, int noOfSstables) throws Exception {
        Path dataDir = Paths.get(configuration.getDataFileLocation());
        BackupFileUtils.generateDummyFiles(dataDir, noOfKeyspaces, noOfCf, noOfSstables, "snapshot", "201812310000");
        String[] ext = {"db"};
    ...
    class FailureFileSystem extends NullBackupFileSystem {
        ...
        @Inject
        public FailureFileSystem(IConfiguration configuration, BackupMetrics backupMetrics, BackupNotificationMgr backupNotificationMgr) {
        ...
        protected void downloadFileImpl(Path remotePath, Path localPath) throws BackupRestoreException {
            throw new BackupRestoreException("User injected failure file system error for testing download. Remote path: " + remotePath);
        }

        @Override
        protected long uploadFileImpl(Path localPath, Path remotePath) throws BackupRestoreException {
            throw new BackupRestoreException("User injected failure file system error for testing upload. Local path: " + localPath);
        }
    }
    ...
    class MyFileSystem extends NullBackupFileSystem {
        ...
        @Inject
        public MyFileSystem(IConfiguration configuration, BackupMetrics backupMetrics, BackupNotificationMgr backupNotificationMgr) {
            super(configuration, backupMetrics, backupNotificationMgr);
        }
