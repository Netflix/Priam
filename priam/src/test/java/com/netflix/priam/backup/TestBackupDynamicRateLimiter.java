...
public class TestBackupDynamicRateLimiter {
    ...
    public void sunnyDay() {
        rateLimiter = getRateLimiter(ImmutableMap.of(), NOW, DIR_SIZE);
    ...
    public void pathIsNotASnapshot() {
        rateLimiter = getRateLimiter(ImmutableMap.of(), NOW, DIR_SIZE);
        AbstractBackupPath path = getBackupPath(
    ...
    public void noBackupThreads() {
        ...
        Assert.assertThrows(
            IllegalStateException.class, () -> timePermitAcquisition(getBackupPath(), LATER, 20));
    }
    ...
    @Test
    public void noData() {
        rateLimiter = getRateLimiter(ImmutableMap.of(), NOW, 0);
        Stopwatch timer = timePermitAcquisition(getBackupPath(), LATER, 20);
        assertNoRateLimiting(timer);
    }
    ...
    public void noPermitsRequested() {
        rateLimiter = getRateLimiter(ImmutableMap.of(), NOW, DIR_SIZE);
        Stopwatch timer = timePermitAcquisition(getBackupPath(), LATER, 0);
        assertNoRateLimiting(timer);
    }
    ...
    private RemoteBackupPath getBackupPath() {
        return getBackupPath("target/data/Keyspace1/Standard1/snapshots/snap_v2_202201010000/.STANDARD1_field1_idx_1/Keyspace1-Standard1-ia-4-Data.db");
    }
