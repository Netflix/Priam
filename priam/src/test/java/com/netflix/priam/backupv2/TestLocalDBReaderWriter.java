...
public class TestLocalDBReaderWriter {
    private static final Logger logger = LoggerFactory.getLogger(TestLocalDBReaderWriter.class.getName());
    ...
    public void readWriteLocalDB() throws Exception {
        ...
        int noOfCf = 1;
        int noOfSstables = 2;
        Path localDbPath = Paths.get(dummyDataDirectoryLocation.toString(), LocalDBReaderWriter.LOCAL_DB);
        ...
        for (int i = 1; i <= noOfKeyspaces; i++) {
            ...
            for (int j = 1; j <= noOfCf; j++) {
                ...
                for (int k = 1; k <= noOfSstables; k++) {
                    ...
                    LocalDBReaderWriter.LocalDB localDB = new LocalDBReaderWriter.LocalDB(new ArrayList<>());
